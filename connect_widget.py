...
class ConnectWidget(QWidget):
    ...
    def __init_ui(self):
        ...
        main_layout.addWidget(self.__refresh_games_button, 0, 1, Qt.AlignBottom)
    ...
    def __connect(self):
        if self.games_table.selectionModel().hasSelection():
            ...
            if sign_in_popup.exec_():
                ...
                self.game_name = self.games_table.item(selected_table_row, 0).text()

                self.hide()
                self.game_window = GraphWidget(self)
                self.game_window.show()
                self.game = Game(self.game_window, self.user_name, self.user_password, self.game_name)
        else:
    ...
    def __create_new_game(self):
        ...
        if new_game_popup.exec_():
            ...
            self.game_window = GraphWidget(self)
            self.game_window.show()
            self.game = Game(self.game_window, self.user_name, self.user_password,
                             self.game_name, self.num_turns, self.num_players)
...
class GamesTable(QTableWidget):
    ...
    def __init__(self, *args):
        ...
        self.setHorizontalHeaderLabels(['Game name', 'Players count', 'Turns', 'State'])
    ...
    def set_data(self):
        ...
        for row, game in enumerate(self.data.values()):
            ...
            self.setItem(row, 0, self.create_table_item(game.name))
            self.setItem(row, 1, self.create_table_item(str(game.num_players)))
            self.setItem(row, 2, self.create_table_item(str(game.num_turns)))
            self.setItem(row, 3, self.create_table_item(game_states[game.state]))
